    ...
    <div class="flex items-center justify-between flex-wrap gap-3">
      <div>
        ...
        <div class="text-xs text-muted mt-0.5">
          {{
            phase === 'select'
              ? 'เลือกงานขนส่งที่ส่งเสร็จแล้ว (DELIVERED) ของลูกค้ารายเดียวกัน — ใบเสร็จจะเริ่มที่สถานะ "รอเก็บเงิน" เสมอ'
              : 'ตรวจสอบ/แก้ไขข้อมูลเอกสารก่อนบันทึกจริง'
          }}
        </div>
      </div>
      <div class="flex items-center gap-2">
        <button v-if="phase === 'select'" @click="router.push('/receipts')" class="btn-secondary">ยกเลิก</button>
        ...
        <button v-if="phase === 'review'" @click="confirmSave" :disabled="saving" class="btn-primary disabled:opacity-40 disabled:cursor-not-allowed">
          ...
          {{ saving ? 'กำลังบันทึก...' : 'ยืนยันสร้างใบเสร็จรับเงิน' }}
        </button>
      </div>
    </div>
          ...
          <table class="w-full text-sm">
            ...
            <tbody>
              ...
              <tr v-if="eligibleBookings.length === 0">
                <td colspan="5" class="px-3 py-6 text-center text-muted">ลูกค้ารายนี้ไม่มีงานที่รอรับเงิน</td>
              </tr>
            </tbody>
          </table>
        </div>
...
/** Create/Edit flow เหมือน BillingCreateFromBookingsView.vue — ยังไม่ persist อะไรจนกว่าจะกด "ยืนยันสร้างใบเสร็จรับเงิน" */
...
const confirmSave = () => {
  ...
  try {
    ...
    const result = salesDocumentsStore.createReceiptFromBookings(orderedIds, {
