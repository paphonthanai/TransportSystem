<template>
  <div class="space-y-4 pb-10">
    <div class="flex items-center justify-between flex-wrap gap-3">
      <div>
        <h2 class="text-lg font-bold text-text">เลือกงานขนส่งเพื่อออกใบเสร็จรับเงิน</h2>
        <div class="text-xs text-muted mt-0.5">เลือกลูกค้าและงานขนส่งที่ส่งเสร็จแล้ว แล้วนำข้อมูลไปแสดงในหน้าสร้างใบเสร็จรับเงิน</div>
      </div>
      <div class="flex items-center gap-2">
        <button @click="router.push('/receipts')" class="btn-secondary">ยกเลิก</button>
    ...
    <div class="card-lg space-y-4">
      ...
      <div v-if="customerName" class="border border-border rounded-lg overflow-x-auto bg-surface">
      ...
      <div v-if="customerName" class="flex justify-end text-sm">
...
/** เงื่อนไขเดียวกับ createReceiptFromBookings/createReceiptManual (claim ทางตรง) ทุกประการ — ไม่มี Feed guard เหมือน Billing/TaxInvoice */
const isBookingBillable = (b: Booking) => b.status === 'DELIVERED' && !b.receiptDocId
...
/** ชนิดปูน (เหมือน BillingBookingSelectView.vue) — ข้อมูลเดียวกับที่ผสมอยู่ใน bookingDescription ด้านบนอยู่แล้ว
...
const goToForm = () => {
  ...
  router.push({ path: '/receipts/new-manual', query: { bookingIds: [...pickerSelectedIds.value].join(',') } })
}
</script>
