  ...
  <div class="space-y-4 pb-10">
    <div class="flex items-center justify-between flex-wrap gap-3">
      ...
      <div class="flex items-center gap-2">
        <button @click="router.push('/quotation')" class="btn-secondary">ปิดหน้าต่าง</button>
...
const editingDoc = editingId ? salesDocumentsStore.documents.find((d) => d.id === editingId && d.type === 'QUOTATION') : undefined
...
const paymentTermMode = ref<PaymentTermMode>('CREDIT_DAYS')
...
const previewNumber = computed(() => {
  ...
  const seq = salesDocumentsStore.documents.filter((d) => d.type === 'QUOTATION').length + 1
  const now = new Date()
...
/** เลขที่เอกสารแก้ไขเองได้ — ตั้งต้นจากเลขที่ auto-generate (หรือเลขเดิมถ้าเป็นการแก้ไข) แล้วผู้ใช้พิมพ์ทับได้อิสระ */
...
const saveAndGetDoc = () => {
  ...
  if (currentId.value) {
    return salesDocumentsStore.updateQuotation(currentId.value, payload)
  }
  const created = salesDocumentsStore.createQuotation(payload)
