...
export interface JobItem {
  ...
  /** วันที่ลงสินค้าของจุดส่งนี้โดยเฉพาะ (ไม่บังคับ) — คนละอย่างกับ deliveredAt (เวลาที่กดส่งสำเร็จจริงในแอปคนขับ)
   *  ฟิลด์นี้ไว้ระบุ/วางแผนวันที่ลงสินค้าล่วงหน้า ข้อมูลเก่าก่อนมี field นี้ไม่มีค่า แสดง "-" แทน */
  loadingDate?: Date
  /** เวลาลงสินค้าของจุดส่งนี้โดยเฉพาะ รูปแบบ "HH:mm" (ไม่บังคับ) ข้อมูลเก่าก่อนมี field นี้ไม่มีค่า แสดง "-" แทน */
  loadingTime?: string
...
export interface Booking {
  ...
  shipDate?: Date
  /** วันที่ + เวลาลงสินค้าโดยรวมของงานนี้ (ไม่บังคับ) — ภาพรวมระดับงาน แยกจาก JobItem.loadingDate/loadingTime ของแต่ละจุดส่ง
   *  = วัน/เวลาที่นัดกับลูกค้าเพื่อส่ง/ลงสินค้า ณ ปลายทาง ข้อมูลเก่าก่อนมี field นี้ไม่มีค่า แสดง "-" แทน ไม่ทำลาย/ไม่ต้องมีก็ได้ */
  loadingDate?: Date
  loadingTime?: string
